...
### Які види рядків бувають у пітоні

Залежить від версії Пітона. У другій гілці два типи: однобайтні рядки та Юнікод представлені класами str та unicode відповідно. У третьому Пітоні є один вид рядків str, який є Юнікодом. Однобайтних рядків немає, замість них є тип bytes, тобто ланцюжок байт.
...
Декодувати – відновити рядок із ланцюжка байт. Викликати метод `.decode()` у об'єкту `str` або `bytes` (версії Пітона 2 і 3 відповідно).

В обох випадках явно передавати кодування, інакше буде використано те, що визначено в системі за промовчанням. Бути готовим зловити винятки `UnicodeEncodeError`, `UnicodeDecodeError`.
...
Послідовності однакових типів порівнювати. Порівняння відбуваються в лексикографічному порядку: послідовність меншої довжини менше, ніж послідовність більшої довжини, якщо їх довжини рівні, то результат порівняння дорівнює результату порівняння перших елементів.
...
Всі стандартні незмінні об'єкти, що хешуються. Всі стандартні об'єкти, що змінюються, не хешуються.
...
- `iter(dictview)` – отримання ітератора за ключами, значеннями або парами ключів та значень. Усі уявлення словників при итерировании повертають елементи словника у порядку. При спробі змінити словник під час ітерування може виникнути виключення RuntimeError
...
Ключом словника може бути будь-який об'єкт, що хешується незмінний: число, рядок, datetime, функція і навіть модуль. Такі об'єкти мають метод `__hash__()`, який однозначно зіставляє об'єкт із деяким числом. За цим числом словник шукає значення ключа.
...
містить глибоко всередині словник, тому хеш може бути розрахований.
...
Функція `zip` віддає список пар N-их елементів. Конструктор `dict` приймає перелік пар. Кожну пару він розглядає як ключ та значення відповідно.

### Як працює хеш-таблиця

Хеш-таблиця це розріджений масив (масив, у якому є незаповнені позиції). У стандартних англомовних підручниках осередку хеш-таблиці називаються "bucket". У хеш-таблиці dict кожному елементу відповідає осередок, що містить два поля: посилання ключ і посилання значення елемента. Оскільки розмір всіх осередків однаковий, доступ до окремої комірки здійснюється за усунення.

Python прагне залишити не менше третини осередків порожніми; якщо хеш-таблиця стає надмірно заповненою, то вона копіюється в нову ділянку пам'яті, де є місце для більшої кількості осередків.
...
Виняток працює лише для дуже маленьких списків довжиною до 5 елементів. У цьому випадку інтерпретатору швидше пробігтиметься по списку, ніж вважати хеш.
...
У Python 3 ці методи створюють об'єкт-виставу. Це напевно відбувається швидше, ніж створення нового списку в Python2. Але ітерування за таким поданням має відбуватися трохи довше, ніж за списком через те, що дані в словниках зберігаються розріджено (рідко, негусто). На підтвердження вищесказаного (Python 3):
...
Можна, функція в Пітон об'єкт першого порядку: допускає привласнення, передачу в функцію, видалення.
...
Лямбди в Пітон можуть складатися тільки з одного виразу. Використовуючи синтаксис
...
У пітоні є оператор `with`. Розміщений усередині код виконується з особливістю: до і після гарантовано спрацьовують події входу в блок `with` та виходу з нього. Об'єкт, який визначає цю логіку, називається контекстним менеджером.
...
Декоратор у сенсі - патерн проектування, коли один об'єкт змінює поведінка іншого. У Пітон декоратор, як правило, це функція A, яка приймає функцію B і повертає функцію C. При цьому функція C задіює в собі функцію B.
...
`type` це метаклас, який Пітон внутрішньо використовує для створення всіх класів.
...
- Якщо Пітон не знаходить `__metaclass__`, він шукає `__metaclass__` у батьківському класі Bar і спробує зробити те саме.
- Якщо ж `__metaclass__` не знаходиться в жодному з батьків, Пітон шукатиме `__metaclass__` на рівні модуля.
...
По суті, GIL у пітоні робить марною ідею застосовувати потоки для паралелізму у обчислювальних задачах. Вони працюватимуть послідовно навіть на багатопроцесорній системі. На CPU Bound завдання програма не прискориться, а тільки сповільниться, так як тепер потокам доведеться ділити навпіл процесорний час. При цьому I/O операції GIL не уповільнить, оскільки перед системним викликом потік відпускає GIL.
...
### Як у пітоні реалізується багатопоточність. Якими модулями
...
### Треди в Пітоні - це нативні треди чи ні
...
## Які варіанти реалізації шаблону Singleton на пітоні
...
Є 5 кейсів використання підкреслюючи в Python:
...
- одинарна на початку або в кінці назви
- подвійне на початку
- подвійне на початку та наприкінці
4. Для використання як функція локалізації
...
Стандартний інтерпретатор пітона (CPython) використовує відразу два алгоритми складання сміття, підрахунок посилань та generational garbage collector (далі GC), більш відомий як стандартний модуль gc з Python.

Алгоритм підрахунку посилань дуже простий і ефективний, але має один великий недолік. Він вміє визначати циклічні посилання. Саме через це, в пітоні існує додатковий збирач, що має назву поколінний GC, який стежить за об'єктами з потенційними циклічними посиланнями.
...
- Django надає пакет "все включено": у вас є панель адміну, інтерфейси баз даних, ORM, та структура каталогів для ваших додатків та проектів.
...
Common Gateway Interface. Угода про те, як веб-сервер взаємодіє з програмою, написаною якоюсь мовою. Веб-сервер запускає програму як файл, що виконується. Параметри запиту, наприклад метод, шлях, заголовки і т.д. передаються через змінні оточення.
...
Завдання, яке традиційно вирішується за допомогою протоколу HTTP — обмін даними між додатком користувача, що здійснює доступ до веб-ресурсів (зазвичай це веб-браузер) і веб-сервером. На даний момент завдяки протоколу HTTP забезпечується робота Всесвітньої павутини.
...
HTTP — прикладний протокол передачі, який використовується для отримання інформації з веб-сайтів.
...
**Hotfix Branches**
Hotfix гілки використовуються для швидкого патчингу релізів. Це єдина гілка, що створюється з master. Як тільки фікс готовий, він повинен бути злитий як в master, так і в development (або в новий release, якщо вона є) і master повинен бути тегованою версією ( інкрементована patch цифра. v.1.0.1, наприклад)
...
- SAVEPOINT: Створює точку якої група транзакцій може відкотитися
